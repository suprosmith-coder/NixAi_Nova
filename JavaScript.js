/* ═══════════════════════════════════════════════════
   ...
   Full rewrite: fixed buttons, 3D Three.js galaxy,
   D3 cobweb search, Supabase auth.
═══════════════════════════════════════════════════ */
...
document.addEventListener('DOMContentLoaded', () => {
  // Bind all buttons first — before any async work
  // This way if Supabase fails, buttons still work
  bindAllButtons();

  // Start 3D galaxy animation
  initGalaxy();
  animCounter('counter', 1847293, 2400);

  // Init Supabase auth (async, non-blocking)
  initSupabase();
});

// ════════════════════════════════════════════════
// BUTTON BINDINGS — all in one place, null-safe
// ════════════════════════════════════════════════
function bindAllButtons() {
  ...
  document.querySelectorAll('.atab').forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      // Show/hide forms using display style (avoids class/CSS conflicts)
  ...
  document.querySelectorAll('.qtag').forEach(tag => {
    tag.addEventListener('click', () => {
      ...
      if (input) { input.value = tag.dataset.q; doSearch(); }
    });
  });
}
...
function show(id, flex)     { const e=el(id); if(e) e.style.display = flex||'block'; }
...
function showErr(id, msg)   { const e=el(id); if(!e) return; e.textContent=msg; e.style.display='block'; }
...
function initSupabase() {
  try {
    // supabase global from CDN UMD bundle
    ...
    // Auth won't work but the rest of the app still runs
  }
}
...
async function oauthLogin(provider) {
  if (!_sb) { alert('Authentication unavailable. Please try again later.'); return; }
...
function showCobweb() {
  hide('screen-galaxy');
  show('screen-cobweb');
  // Init cobweb only after screen is visible so canvas sizes correctly
...
function buildClusters(scene) {
  ...
  CLUSTERS.forEach(cl => {
    ...
    const nMat = new THREE.MeshBasicMaterial({
      color:new THREE.Color(cl.color), transparent:true, opacity:.05,
      blending:THREE.AdditiveBlending, depthWrite:false, side:THREE.BackSide,
    });
    group.add(new THREE.Mesh(new THREE.SphereGeometry(.6,16,16), nMat));
...
function bindGalaxyEvents(canvas, renderer, camera, raycaster) {
    ...
    for (const p of _interactable) {
      ...
      if (raycaster.intersectObject(p.mesh).length > 0) {
        if (_locked && _locked.site === p.site) {
          ...
          _locked = p;
          showLockOn(p, camera, renderer.domElement);
          _autoRot = false;
          const wp = p.mesh.getWorldPosition(new THREE.Vector3()).normalize();
          _tSph.theta = Math.atan2(wp.x, wp.z);
          _tSph.phi   = Math.acos(Math.max(-1, Math.min(1, wp.y)));
          _tSph.r     = Math.max(2.8, _sph.r * .75);
  ...
  canvas.addEventListener('touchmove', e => {
    if (e.touches.length===1 && _isDragging) {
      ...
      if (Math.hypot(dx,dy)>3) _dragMoved=true;
      _tSph.theta -= dx*.006;
      _tSph.phi    = Math.max(.12,Math.min(Math.PI-.12,_tSph.phi+dy*.006));
  ...
  (function loop(ts) {
    ...
    scene.children.forEach((c, i) => {
      if (c.userData.clusterId) {
        const idx = CLUSTERS.findIndex(cl => cl.id === c.userData.clusterId);
        c.rotation.y = t * .035 * (idx%2===0?1:-1);
        c.rotation.x = Math.sin(t*.018+idx)*.04;
      }
    ...
    if (!_locked) {
      ...
      if (found !== _hovered) {
        _hovered = found;
        updateTooltip(found, camera, renderer.domElement);
      }
    } else {
      updateLockOnPos(_locked, camera, renderer.domElement);
      _hovered = null;
    }

    const canvas = renderer.domElement;
    canvas.style.cursor = (_locked||_hovered) ? 'pointer' : (_isDragging ? 'grabbing' : 'grab');
...
function updateTooltip(found, camera, canvas) {
  ...
  setText('tt-cluster', cl.label);
  const c = el('tt-cluster'); if(c) c.style.color=cl.color;
  ...
  tt.style.top     = Math.max(py-60, 8)+'px';
...
function showLockOn(p, camera, canvas) {
  ...
  const c = el('lo-cluster'); if(c) c.style.color=cl.color;
  ...
  if (launch) {
    if (site.u && site.u !== '#') {
      ...
      launch.style.borderColor = cl.color;
      launch.style.color       = cl.color;
      launch.onclick           = () => window.open(site.u, '_blank', 'noopener');
    } else {
  ...
  panel.style.transform = 'translate(-50%,-50%) scale(.88)';
  requestAnimationFrame(() => {
    panel.style.transition = 'opacity .22s ease, transform .28s cubic-bezier(.34,1.56,.64,1)';
...
function hideLockOn() {
  ...
  if (!panel) return;
  panel.style.opacity   = '0';
  panel.style.transform = 'translate(-50%,-50%) scale(.9)';
...
// ════════════════════════════════════════════════
// COBWEB / SEARCH (D3 force graph)
// ════════════════════════════════════════════════
let _cwReady = false;
let _cwSim   = null;
let _cwNodes = [], _cwLinks = [];
let _cwW     = window.innerWidth;
let _cwH     = window.innerHeight;
let _bgParts = [];
...
function initBgParticles() {
  ...
  _bgParts = Array.from({length:100}, () => ({
    x:Math.random()*_cwW, y:Math.random()*_cwH,
    vx:(Math.random()-.5)*.3, vy:(Math.random()-.5)*.3,
    r:Math.random()*1.2+.4, a:Math.random()*.3+.08,
  }));
  (function draw() {
    ...
    for(let i=0;i<_bgParts.length;i++) {
      for(let j=i+1;j<_bgParts.length;j++) {
        ...
        if(d<80) {
          ...
          ctx.strokeStyle=`rgba(0,240,255,${.05*(1-d/80)})`;
...
function cwResize() {
  ...
  const c=el('bg-canvas');
  if(c) { c.width=_cwW; c.height=_cwH; }
...
async function doSearch() {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Search returned ${res.status}: ${txt.slice(0,120)}`);
    }
...
function cwSimulate() {
  ...
  node.append('circle')
    .attr('r', d=>d.isCenter?22:nodeR(d))
  ...
  node.append('text')
    ...
    .attr('dy', d=>d.isCenter?38:nodeR(d)+14)
    .text(d=>d.label);

  // Pre-run ticks then animate in
  ...
  link.transition().duration(700).delay(160).attr('opacity',d=>{
    const s=typeof d.source==='object'?d.source:null;
    return s?.isCenter?.5:.2;
  });
}

function nodeR(d)  { return 6 + (d.weight||5)*1.1; }
...
function animCounter(id, target, ms) {
  ...
  (function step(now) {
    ...
    e.textContent=Math.floor(target*(1-Math.pow(1-p,3))).toLocaleString();
